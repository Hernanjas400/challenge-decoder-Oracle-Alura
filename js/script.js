function encriptar(traduccion){
    document.querySelector("#Check").removeAttribute("style");
    ...
    if (texto != ""){
        ...
        for(var i=0; i < texto.length; i++){
            if(((texto[i] < 'a') || (texto[i] > 'z')) && (texto[i] != ' ')){
                ...
                area_default.classList.remove("transform");
                area_result.classList.add("transform");
                return;
            }
            if(texto[i] == 'a'){
                out += traduccion["a"] ;
        ...
        area_default.classList.add("transform");
        area_result.classList.remove("transform");
...
copy.addEventListener( 'click', function() {clipboard(); alert(" Se Copia!!!!");
} );
